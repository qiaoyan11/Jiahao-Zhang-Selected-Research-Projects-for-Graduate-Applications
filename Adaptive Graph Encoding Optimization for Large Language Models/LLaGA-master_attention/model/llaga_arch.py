...
class LlagaMetaModel:
    ...
    def __init__(self, config):
        ...
        self.graph_proj = nn.Linear(2432, config.hidden_size)  # 适应attention

        ###########################################################################################
        ###########################################################################################
        self.mm_attention = nn.MultiheadAttention(
...
class LlagaMetaForCausalLM(ABC):
    ...
    def encode_graphs(self, graph, graph_emb):
        # graph_emb: (batch, seq, dim)
        graph_emb = self.get_model().graph_proj(graph_emb)


        print("graph_emb shape:", graph_emb.shape)
        attn_output, _ = self.get_model().mm_attention(graph_emb, graph_emb, graph_emb)



        graph_features = attn_output
        graph_features[graph == DEFAULT_GRAPH_PAD_ID] = 0.
        return graph_features
