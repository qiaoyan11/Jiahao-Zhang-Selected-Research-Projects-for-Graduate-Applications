...
###############################################################################################
###############################################################################################
def get_random_walk_sequence(edge_list, node_idx, walk_length, num_walks, top_k):
    """
    使用Random Walk采样获取节点序列
    Args:
        edge_list: 图的边列表
        node_idx: 起始节点
        walk_length: 每次游走的长度
        num_walks: 游走次数
        top_k: 返回访问频次最高的前k个节点
    """
    node_visits = {}  # 记录节点访问次数

    for _ in range(num_walks):
        current_node = node_idx
        for _ in range(walk_length):
            if current_node == DEFAULT_GRAPH_PAD_ID:
                break
            neighbors = edge_list[current_node]
            if not neighbors:
                break
            current_node = random.choice(neighbors)
            node_visits[current_node] = node_visits.get(current_node, 0) + 1
...
def get_fix_shape_subgraph_sequence(edge_list, node_idx, template_type=DEFAULT_TEMPLATE, **kwargs):
    """
    统一的采样接口，根据template_type选择不同的采样方式
    """
    if template_type == TEMPLATE_ND:
        return get_fix_shape_subgraph_sequence_fast(
            edge_list,
            node_idx,
            kwargs.get('k_hop', 2),
            kwargs.get('sample_size', 10),
            kwargs.get('avoid_idx', None)
        )
    elif template_type == TEMPLATE_RW:
        return get_random_walk_sequence(
            edge_list,
            node_idx,
            kwargs.get('walk_length', 10),
            kwargs.get('num_walks', 5),
            kwargs.get('top_k', 50)
        )
    else:
